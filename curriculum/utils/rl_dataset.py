...
class AutoGeneratedRLHFDataset(RLHFDataset):
    ...
    def _load_good_examples(self, top_n: int = 3) -> List[Dict[str, str]]:
        ...
        if questions_dir is None or not os.path.exists(questions_dir):
            print(f"⚠️⚠️⚠️ the question dir {questions_dir} does not exist, return empty list")
            return []
        
        # Get all results_sc_*.json files sorted by modification time (newest first)
        try:
            files = [f for f in os.listdir(questions_dir) if f.startswith("results_sc_") and f.endswith(".json")]
            if not files:
                print(f"⚠️⚠️⚠️ No results_sc_*.json files found in {questions_dir}")
                return []
            
            # Sort by modification time, newest first
            files.sort(key=lambda x: os.path.getmtime(os.path.join(questions_dir, x)), reverse=True)
            
            # Take only the last 5 files
            files = files[:5]
            
            # Collect all questions with sc_score == 0.5
            good_examples = []
            for file in files:
